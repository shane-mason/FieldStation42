...
class ReelCutter:
    ...
    def cut_reels_into_base(base_clip, reel_blocks, base_offset, base_duration, break_strategy, start_bump, end_bump, break_points=None):
        ...
        if start_bump:
            entries.append(BlockPlanEntry(start_bump["path"], 0, start_bump["duration"], content_type="bump"))
        ...
        if break_count <= 1 or break_strategy == "end" or break_strategy == "center":
            if break_strategy == "center":
                ...
                h1 = BlockPlanEntry(base_clip.path, base_offset, h_dur, content_type=base_clip.content_type)
                h2 = BlockPlanEntry(base_clip.path, base_offset+h_dur, h_dur, content_type=base_clip.content_type)
                entries.append(h1)
            else:
                # then don't cut the base at all
                entries.append(BlockPlanEntry(base_clip.path, base_offset, base_duration, content_type=base_clip.content_type))
            ...
            if not break_points:
                for i in range(break_count):
                    entries.append(BlockPlanEntry(base_clip.path, offset, segment_duration, content_type=base_clip.content_type))
                ...
                if len(break_points):
                    ...
                    entries.append(BlockPlanEntry(base_clip.path, first_segment["chapter_start"], segment_duration, content_type=base_clip.content_type))
                ...
                while keep_going:
                    ...
                    if len(break_points):
                        ...
                        e = BlockPlanEntry(base_clip.path, this_bp["chapter_start"], segment_duration, content_type=base_clip.content_type)
                        entries.append(e)
        ...
        if end_bump:
            entries.append(BlockPlanEntry(end_bump["path"], 0, end_bump["duration"], content_type="bump"))
    ...
    def cut_reels_into_clips(clips, reel_blocks, break_stategy, start_bump, end_bump):
        ...
        if break_count <= 1 or break_stategy == "end":
            # then don't cut the base at all
            for clip in clips:
                entries.append(BlockPlanEntry(clip.path, 0, clip.duration, content_type=clip.content_type))
            for _block in reel_blocks:
                # and put the reel at the end if there is one
                entries += _block.make_plan()
