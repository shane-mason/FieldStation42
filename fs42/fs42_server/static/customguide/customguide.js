...
function startScrolling() {
  ...
  if (LOOP) {
    ...
    clone.id = 'guide-scroll-strip-clone';
    ...
    if (RANDOM_START) {
    scrollY = RANDOM_START ? Math.random() * originalHeight : 0;
    } else {
        scrollY = 0;
    }
...
document.addEventListener('DOMContentLoaded', init);const params = new URLSearchParams(window.location.search);
const THEME = params.get('theme') || '80s';
const SLOT_COUNT = parseInt(params.get('slots')) || 3;
const HEADER_TEXT = params.get('header') || null;
const PAUSE_OVERRIDE = params.get('pause');
