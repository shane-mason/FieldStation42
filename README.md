...
FieldStation42 uses a directory (folder) structure to tag content per station. Each station has a content directory (folder) specified in its configuration. FieldStation42 will scan the content directory and build a catalog based on movie files found in the directories. For example, a station's content folder may have the following sub-folders:
...
FieldStation42 attempts to be as flexible as possible and doesn't force any particular folder names - the names of your folders are completely up to you. Channel configuration will use the folder names as 'tags' to schedule based on the channel configuration - see below for details.
